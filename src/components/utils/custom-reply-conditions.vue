  ...
  <div v-for="(cond, index) in data" :key="index" class="mb-3 border-l-2 border-orange-500 pl-2">
    ...
    <div v-if="cond.condType === 'textMatch'" style="display: flex" class="mobile-changeline">
      <div style="width: 7rem; margin-right: 0.5rem">
        ...
        <el-select v-model="cond.matchType" size="small" placeholder="选择方式">
          <el-option
            v-for="item in [
    ...
    <div v-else-if="cond.condType === 'exprTrue'" style="display: flex" class="mobile-changeline">
      <div style="flex: 1">
        ...
        <el-input v-model="cond.value" type="textarea" :autosize="{ minRows: 1, maxRows: 10 }" />
      </div>
    </div>

    <el-space v-else-if="cond.condType === 'textLenLimit'" class="mt-2 flex">
      <el-radio-group v-model="cond.matchOp" size="small">
        <el-radio-button value="ge" label="大于等于" />
        <el-radio-button value="le" label="小于等于" />
      </el-radio-group>
      <el-input-number v-model="cond.value as number" :min="0" />
    </el-space>
  </div>
</template>
